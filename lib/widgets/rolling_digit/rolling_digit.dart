...
class NP {
  ...
  static void enableBoundaryChecking([bool flag = true]) {
...
class ValueColor {
  ...
  ///
  /// #### the value text color
  ValueColor({required this.condition, required this.color});
}
...
class AnimatedDigitWidget extends StatefulWidget {
  ...
  @override
  AnimatedDigitWidgetState createState() {
    return AnimatedDigitWidgetState();
  }
}
...
class AnimatedDigitWidgetState extends State<AnimatedDigitWidget> with WidgetsBindingObserver {
  ...
  String _getFormatValueAsString() {
    return _formatNum(_value.toString(), fractionDigits: widget.fractionDigits);
  }
  ...
  void _setValue(Key? aswKey, String value) {
    assert(aswKey != null);
    if (aswKey is GlobalKey<_AnimatedSingleWidgetState>) {
      aswKey.currentState?.setValue(value);
    }
  }
...
class _AnimatedSingleWidgetState extends State<_AnimatedSingleWidget> {
  ...
  @override
  Widget build(BuildContext context) {
    late Widget child = Center(widthFactor: 1.0, child: _build());
  ...
  Widget _buildSingleWidget(String val) {
    ...
    if (!sdd.useTextSize && sdd.size != null) {
      child = SizedBox.fromSize(size: valueSize, child: child);
    }
    return child;
  }
}
